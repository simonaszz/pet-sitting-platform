          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pets.map((pet) => (
              <PetCard
                ...
                onEdit={(target) => setEditingPet(target)}
              />
            ))}
          </div>
        )}
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Redaguoti augintinį</h2>
            <button
              type="button"
              onClick={onClose}
...
function AddPetModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleChange = (formChangeEvent: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value } = formChangeEvent.target;
    setFormData(prev => ({
      ...prev,
      [name]: name === 'age' ? (value ? parseInt(value) : undefined) : value,
